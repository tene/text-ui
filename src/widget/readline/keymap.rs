...
impl InputState {
    ...
    pub fn next_cmd(&mut self, key: KeyPress, wrt: &mut Refresher, single_esc_abort: bool) -> Cmd {
        match self.mode {
            EditMode::Emacs => self.emacs(key, wrt, single_esc_abort),
            EditMode::Vi if self.input_mode != InputMode::Command => self.vi_insert(key, wrt),
            EditMode::Vi => self.vi_command(key, wrt),
        }
    }

    fn emacs(&mut self, key: KeyPress, _wrt: &mut Refresher, _single_esc_abort: bool) -> Cmd {
    ...
    fn vi_command(&mut self, key: KeyPress, wrt: &mut Refresher) -> Cmd {
    ...
    fn vi_insert(&mut self, key: KeyPress, wrt: &mut Refresher) -> Cmd {
        if let Some(cmd) = self.custom_bindings.borrow().get(&key) {
            debug!(target: "rustyline", "Custom command: {:?}", cmd);
            return if cmd.is_repeatable() {
                cmd.redo(None)
    ...
    fn vi_cmd_motion(
        ...
        if mvt == key {
            return Some(Movement::WholeLine);
        }
        match mvt {
